...
def test_product_batch_dao(conexao):
    ...
    print("\n==============================")
    print("Testes do DAO ProductBatchDAO")
    print("==============================\n")
    ...
    def print_list_batches(dao, filters):
        try:
    ...
    cat_dao.create_new(Category(name="Cosméticos"))

    cat_med = get_cat(conexao, 1)
    cat_cosm = get_cat(conexao, 2)

    prod_dao.create_new(Product("Paracetamol", "NeoQuímica", cat_med, 20.0))
    ...
    print("\nTestes de create_new()")
    teste_metodo_by_id_dao(lambda: dao.create_new(ProductBatch(product=prod_paracetamol, batch_number="L1", manufacturing_date=Date(1, 1, 2023), quantity=100)), "create_new com lote válido com dados mínimos (sem data de validade) (novo lote #L1 do produto 'Paracetamol' da marca 'NeoQuímica' esperado)")
    ...
    teste_metodo_by_id_dao(lambda: dao.create_new(ProductBatch(product=prod_paracetamol, batch_number="", manufacturing_date=Date(1, 1, 2023), quantity=100)), "create_new com número de lote vazio (ERRO esperado)")
    ...
    print("\nTestes de update_by_id()")
    teste_metodo_by_id_dao(lambda: dao.update_by_id(1, ProductBatch(prod_paracetamol, batch_number="L1", manufacturing_date=Date(1, 1, 2023), quantity=80)), "update_by_id válido (Lote #L1 de produto 'Paracetamol', marca 'NeoQuímica' atualizado esperado)")
    ...
    print("\nTestes de delete_by_id()")
    teste_metodo_by_id_dao(lambda: dao.create_new(ProductBatch(product=prod_paracetamol, batch_number="TEMP", manufacturing_date=Date(1, 1, 2022), quantity=2)), "novo lote #TEMP do produto 'Paracetamol' da marca 'NeoQuímica'")
    ...
    print("\nTestes de find_by_id()")
    teste_metodo_by_id_dao(lambda: dao.find_by_id(1), "find_by_id com dado (id) válido (lote 'L1' de 'Paracetamol', marca 'NeoQuímica' esperado)")
    teste_metodo_by_id_dao(lambda: dao.find_by_id(99), "find_by_id com ID inexistente (ERRO esperado)")

    # testes de list()
    print("\nTestes de list()")

    # Garante lotes distintos para testes de list()
    ...
    dao.create_new(ProductBatch(product=prod_paracetamol, batch_number="PARA-VALIDO", manufacturing_date=Date(1,1,2023), due_date=Date(1,1,2026), quantity=20))
    ...
    print_list_batches(dao, filters)
    i+=1

    print(f"\n[{i}] list() com produto #1 (apenas os lotes do produto de ID 1) (lista populada esperada)")
    filters = dict(product_id=1)
    print_list_batches(dao, filters)
    i+=1

    print(f"\n[{i}] list() com keyword 'SAB'")
    filters = dict(keyword="SAB")
    print_list_batches(dao, filters)
    i+=1

    print(f"\n[{i}] list() com quantidade mínima 10")
    filters = dict(min_quantity=10)
    print_list_batches(dao, filters)
    i+=1

    print(f"\n[{i}] list() com quantidade máxima 5")
    ...
    print(f"\n[{i}] list() com ordenação por coluna inválida (lista ordenada por id esperada)")
    filters = dict(order_by_type='order')
    print_list_batches(dao, filters)
    i+=1


    print(f"\n[{i}] list() com ordenação por direção inválida (lista ordenada em direção descendente esperada)")
